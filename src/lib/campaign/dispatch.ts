...
import { cancelPending, isSuppressed, moveStage, recordEvent, stoppedStages, suppress } from './state'
...
async function scheduleNextStep(db: Database, message: MessageRow, config: CampaignConfig, sentAt: Date, random: () => number): Promise<void> {
...
async function processMessage(db: Database, message: MessageRow, context: ProcessContext): Promise<Outcome> {
  ...
  if (!result.error) {
    await db
      .update(messages)
      .set({ status: 'sent', sentAt: now, subject: rendered.subject, rfcMessageId, providerId: result.providerId, senderId: sender.id, lastError: null, updatedAt: now })
    ...
    if (!message.isTest) {
      await moveStage(db, contact.id, 'contacted')
      await scheduleNextStep(db, message, config, now, context.random)
    }
    return { kind: 'sent' }
  }
...
async function sendWhatsapp(
  ...
  if (!result.error) {
    await db.update(messages).set({ status: 'sent', sentAt: now, providerId: result.providerId, lastError: null, updatedAt: now }).where(eq(messages.id, message.id))
    await recordEvent(db, { contactId: contact.id, messageId: message.id, type: 'wa_sent', data: { mode: 'cloud' }, at: now })
    await moveStage(db, contact.id, 'contacted')
    return { kind: 'sent' }
  }
  if (!result.permanentFailure && message.attempts < MAX_ATTEMPTS) {
    await reschedule(db, message, new Date(now.getTime() + message.attempts * 10 * 60 * 1000), result.error)
    return { kind: 'rescheduled', error: result.error }
  }
  await db.update(messages).set({ status: 'failed', lastError: result.error, updatedAt: now }).where(eq(messages.id, message.id))
